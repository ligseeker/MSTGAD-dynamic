...
class Process:
    ...
    def load_raw(self):
        ...
        for _, row in tqdm(log_df_valid.iterrows(), total=len(log_df_valid), desc='Filling log matrix'):
            ts = int(row['timestamp'])
            svc_idx = int(row['svc_idx'])
            values = row[use_template_cols].values.astype(float)
            log_record[ts][svc_idx, :] = values
